...
class TravelViewSafe(discord.ui.View):
    ...
    async def _render(self):
        ...
        embed.set_footer(text=f"Página {self.page + 1} / {max(1, math.ceil(len(self._dest_cache) / MAX_DEST_PER_PAGE))}")
...
# Se você já tem um PlayerAdapter em outro arquivo, remova esta classe duplicada.
class PlayerAdapter:
...
class AdventureCog(commands.Cog):
    ...
    async def cmd_travel(self, ctx: commands.Context, *, apenas_principal: Optional[bool] = False):
        ...
        - Lê o jogador do seu sistema (substitua pelo seu adapter real)
        - Exibe embed imediatamente e carrega destinos em seguida
        """
        # >>> Troque isso pelo seu mecanismo real de player <<<
        # Exemplo: buscar no BD. Aqui só criamos um "stub" com defaults.
