...
CANVAS_WIDTH = 900
CANVAS_HEIGHT = 600
BG_COLOR = (44, 47, 51) # Cor de fundo escura, similar ao Discord
TEXT_COLOR = (255, 255, 255) # Texto branco
HP_BAR_BG_COLOR = (100, 100, 100) # Cinza para o fundo da barra de HP
MOVES_BOX_COLOR = (70, 75, 78) # Cor das caixas de ataques
PARTY_BOX_COLOR = (70, 75, 78) # Cor das caixas dos outros Pokémon
SELECTED_PARTY_COLOR = (88, 101, 242) # Azul para o Pokémon selecionado na party

# Caminhos das fontes
try:
    # Ajuste o caminho se a pasta assets não estiver na raiz do seu projeto
...
async def create_team_image(focused_pokemon: dict, full_team_db: list, focused_slot: int) -> BytesIO:
    ...
    full_team_db: Lista completa de pokémon do jogador (dados do DB)
    focused_slot: Posição do pokémon focado
    """
    ...
    if main_sprite:
        # Posição do sprite principal (um pouco acima do centro)
    ...
    hp_bar_y = 350 # Posição Y da barra de HP
    ...
    # Parte preenchida da barra de HP
    ...
    draw.text(
        (CANVAS_WIDTH // 2, hp_bar_y + hp_bar_height + 5),
        hp_text,
        font=FONT_SMALL,
        fill=TEXT_COLOR,
        anchor="mt"
    ...
    for i in range(4):
        ...
        # Ajustar o tamanho da fonte se o nome do ataque for muito longo
        ...
        draw.text(
            (box_x + move_box_size // 2, box_y + move_box_size // 2),
            move_name,
            font=move_font,
            fill=TEXT_COLOR,
            anchor="mm" # Âncora no "meio-meio" (centraliza o texto na caixa)
        )
    ...
    # Calcular a largura total dos slots para centralizá-los
    ...
    # Assumimos que a ordem em full_team_db já está correta (ordenada por party_position)
    for i, p_mon_db in enumerate(full_team_db):
        ...
        # Desenha o sprite pequeno dentro da caixa
        # Precisamos buscar o sprite_url do API, não temos ele no full_team_db
        ...
        if p_mon_api_data:
            ...
            small_sprite = await _get_sprite(sprite_url_small, (party_slot_size - 10, party_slot_size - 10)) # Pequeno, com margem
            if small_sprite:
                # Centraliza o sprite na caixa
        ...
        # Desenha o nível abaixo da caixa
        draw.text(
